...
description: Use when editing Arc/Circle/Canteen integration, Circle SDK calls, Arc Testnet config, USDC transfers, ERC-8004 identity/reputation, ERC-8183 jobs, identity/*.ts, jobs/*.ts, agent/on_chain.py, or contracts/arc_addresses.py. Enforces USDC 6-decimal ERC-20 conversions and judge-before-chain execution. Do not use for alpha scoring.
---
...
open contracts/arc_addresses.py, agent/on_chain.py, identity/*.ts, jobs/*.ts, AGENTS.md
use ERC-20 USDC interface with 6 decimals
always confirm decimals before amount conversion
never mix native 18-decimal gas accounting with ERC-20 6-decimal USDC arithmetic
never call Arc unless the judge returned EXECUTE
use ERC-8183 for job lifecycle and ERC-8004 for identity/reputation
do not duplicate alpha/scoring logic here

...
open agent/pnl_probe.py, policy.yaml, logs/pnl_reconciliation.tsv, relevant tests, AGENTS.md
do not calibrate on fewer than 10 reconciled positions per surface
bias estimates toward zero
do not change judge semantics, Arc settlement, or the premium gate
do not set any basis below zero
one surface per calibration patch
